...
// FIXME trying out the UpdateOp API

object VotingContest2Spec extends MultiNodeConfig {
...
class VotingService2 extends Actor {
  import akka.contrib.datareplication.Replicator._
  import VotingService2._
  ...
  def receive = {
    case Open ⇒
      ...
      // make sure there is one empty initial value
      replicator ! Update(CountersKey, PNCounterMap.empty)
      becomeOpen()
  ...
  def open: Receive = {
    case v @ Vote(participant) ⇒
      val updateOp = UpdateOp(CountersKey, request = Some(v)) {
        case counters: PNCounterMap => counters.increment(participant, 1)
      }
      replicator ! updateOp
...
class VotingContest2SpecMultiJvmNode1 extends VotingContest2Spec
class VotingContest2SpecMultiJvmNode2 extends VotingContest2Spec
class VotingContest2SpecMultiJvmNode3 extends VotingContest2Spec

class VotingContest2Spec extends MultiNodeSpec(VotingContest2Spec) with STMultiNodeSpec with ImplicitSender {
  import VotingContest2Spec._
  ...
  "Demo of a replicated voting" must {
    ...
    "count votes correctly" in within(15.seconds) {
      import VotingService2._
      val votingService = system.actorOf(Props[VotingService2], "votingService")
      val N = 1000
      runOn(node1) {
        votingService ! Open
        for (n ← 1 to N) {
